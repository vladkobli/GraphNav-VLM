...
class LeRobotRealsenseSequenceLogger(SnapshotCaptureNode):
    ...
    def __init__(self, signals: GuiSignals):
        ...
        # Depth artifact saving, same idea as GraphNavStateMachine.
        if not self.has_parameter("realsense_depth_camera_info_topic"):
            self.declare_parameter(
                "realsense_depth_camera_info_topic",
                "/camera/driver/aligned_depth_to_color/camera_info",
            )

        if not self.has_parameter("depth_save_formats"):
            self.declare_parameter("depth_save_formats", ["png", "npz", "preview_png"])

        if not self.has_parameter("max_depth_preview_m"):
            self.declare_parameter("max_depth_preview_m", 6.0)
        ...
        self.max_depth_preview_m = float(
            self.get_parameter("max_depth_preview_m").value
        )
        ...
        for source in self.image_sources:
            ...
            self.get_logger().info(f"  realsense_depth_camera_info: {depth_camera_info_topic}")
    ...
    def save_realsense_stop_snapshot(self, folder, stop_index, stop_name):
        ...
        for source in self.image_sources:
            ...
            elif name == "realsense_color":
                path = folder / f"c{stop_index}_{safe_stop}.png"
            ...
            if name == "realsense_aligned_depth":
                stamps[name]["depth_scale_to_meters"] = self.depth_scale_to_meters(
                    msg.encoding
                )
                stamps[name]["camera_info"] = self.camera_info_to_dict(
                    depth_camera_info
                )
    ...
    def save_depth_artifacts(self, folder, stop_index, stop_name, depth_image, encoding):
        ...
        if "png" in self.depth_save_formats:
            ...
            if not cv2.imwrite(
                str(path),
                self.prepare_image_for_saving(depth_image, encoding),
            ):
                raise RuntimeError(f"cv2.imwrite failed for {path}")
        ...
        if "npz" in self.depth_save_formats:
            ...
            np.savez_compressed(
                ...
                depth_scale_to_meters=np.array(
                    self.depth_scale_to_meters(encoding),
                    dtype=np.float32,
                ),
            )
    ...
    def depth_scale_to_meters(encoding):
        ...
        # RealSense ROS depth is commonly 16UC1 in millimeters.
        if encoding in ("16uc1", "mono16"):
            return 0.001

        # For 32FC1, values are usually already meters.
        return 1.0
