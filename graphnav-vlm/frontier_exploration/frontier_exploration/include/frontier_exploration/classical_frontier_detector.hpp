...
class FrontierExplorer : public rclcpp::Node
{
    ...
    private:
        int region_size_thresh_ {12};
        int preprocess_iterations_ {0};
        int marker_min_region_size_ {12};
        int marker_max_count_ {35};
        float robot_width_ {0.5};
        float marker_scale_ {0.08};

        std::string occupancy_map_topic_ {"map"}; //"global_costmap/costmap"
        
        std::string map_frame_ {"map"};
        std::string base_frame_ {"panther/base_link"};
        std::string odom_frame_ {"panther/odom"};
