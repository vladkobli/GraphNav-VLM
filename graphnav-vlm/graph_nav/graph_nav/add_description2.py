...
def build_objects_only_prompt() -> str:
    return """
List concrete objects directly visible in this image.
Return only a comma-separated list of objects.
Do not include scene types, places, hidden objects, or explanations.
If no concrete objects are visible, write none.
""".strip()
...
def describe_image(model, image_path: Path, task: str | None = None, target_prompt: str | None = None) -> dict:
    ...
    if visible_objects == "none":
        objects_answer = query_moondream_answer(model, image, build_objects_only_prompt())
        visible_objects = clean_object_list(objects_answer)
        if visible_objects == "none":
            visible_objects = infer_visible_objects(objects_answer)
