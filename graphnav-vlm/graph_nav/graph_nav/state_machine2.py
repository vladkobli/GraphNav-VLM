...
DESCRIPTION_SOURCE = "add_description.py"
...
class GraphNavStateMachine(Node):
    def __init__(self):
        super().__init__("graph_nav_state_machine2")
        ...
        if self.auto_scan_after_nav2_success:
    ...
    def on_target_visible(self, msg):
        ...
        if self.target_visible and self.phase == Phase.EXPLORATION:
    ...
    def apply_description_payload(self, folder, payload):
        ...
        descriptions["description_source"] = DESCRIPTION_SOURCE
    ...
    def generate_candidates(self, folder):
        ...
        candidate_generation = {
            ...
            "updated_at": now,
            "algorithm": (
                "llm_qwen_legacy_description_view_selector_v1"
                if llm_selection.get("status") == "completed"
                else "legacy_description_keyword_affordance_v1"
            ),
            "task": self.user_prompt,
            "target_prompt": self.target_prompt,
            "task_terms": task_terms,
...
""".strip()
    ...
    def build_direction_candidate(self, snapshot, task_terms, repeated_direction_counts):
        ...
        legacy_description = not any(
            key in semantic
            for key in (
                "task_relevance_score",
                "task_relevance_level",
                "best_direction_level",
        ...
        if legacy_description and matched_terms:
            task_relevance_score = min(3, len(matched_terms))
            task_relevance_level = "contextual"
        ...
        if legacy_description and target_phrase:
            has_direct_target = has_direct_target or target_phrase in text_l
        has_direct_evidence = direct_target_visible or (
            has_direct_target and self.has_non_template_target_quote(direct_target_text)
        )
        if legacy_description and has_direct_target:
        ...
        if best_direction_level == "high":
            ...
            score += 1.0
        elif legacy_description and route_affordance in ("open", "weak") and matched_terms:
            score += 1.0
