...
LOCAL_SCAN_VIEWS = [
    {
        "stop_index": 3,
        "stop_name": "left",
        "relative_yaw_deg": 70.0,
        "label": "local_view_left",
    },
    {
        "stop_index": 5,
        "stop_name": "front",
        "relative_yaw_deg": 0.0,
        "label": "local_view_front",
    },
    {
        "stop_index": 7,
        "stop_name": "right",
        "relative_yaw_deg": -70.0,
        "label": "local_view_right",
    },
]
...
class GraphNavStateMachine(Node):
    def __init__(self):
        ...
        self.declare_parameter("next_node_goal_distance_m", 1.0)
        ...
        self.declare_parameter("rotate_before_next_node_goal", True)
        self.declare_parameter("rotation_yaw_tolerance_deg", 4.0)
        self.declare_parameter("rotation_cmd_angular_speed_rad_s", 0.35)
        ...
        self.rotate_before_next_node_goal = bool_parameter(
            self.get_parameter("rotate_before_next_node_goal").value
        )
        self.rotation_yaw_tolerance_rad = math.radians(
            float(self.get_parameter("rotation_yaw_tolerance_deg").value)
        )
        self.rotation_cmd_angular_speed_rad_s = float(
            self.get_parameter("rotation_cmd_angular_speed_rad_s").value
        )
        ...
        self.initial_sweep_completed = False
    ...
    def create_node_and_scan_360(self):
        ...
        try:
            ...
            initial_sweep = not self.initial_sweep_completed
            scan_mode = "initial_360_sweep" if initial_sweep else "local_three_view_scan"

            self.write_initial_metadata(folder, node_id, scan_mode=scan_mode)
            self.get_logger().info(f"create_node: created {folder}")

            self.scan_node_views(folder, initial_sweep=initial_sweep)
            if self.should_stop_scan_for_target():
                self.mark_metadata_status(folder, "target_visible_during_sweep")
                self.get_logger().info(
                    "Target became visible during scan; skipping descriptions, "
            ...
            if initial_sweep:
                self.initial_sweep_completed = True
            self.mark_metadata_status(folder, "completed")
            self.get_logger().info(f"{scan_mode} complete: {folder}")
...
""".strip()
    ...
    def generate_next_node_goal(self, folder):
        ...
        if self.rotate_before_next_node_goal:
            rotation_ok = self.rotate_in_place_to_yaw(goal_yaw)
            goal["pre_goal_rotation"] = {
                "attempted_at": datetime.now(timezone.utc).isoformat(),
                "target_yaw": goal_yaw,
                "target_yaw_deg": math.degrees(goal_yaw),
                "ok": rotation_ok,
                "tolerance_deg": math.degrees(self.rotation_yaw_tolerance_rad),
            }
            metadata["next_node_goal"] = goal
            self.write_json(path, metadata)
    ...
    def build_direction_candidate(self, snapshot, task_terms, repeated_direction_counts):
        ...
        rel_yaw = snapshot.get("relative_yaw_deg")
        if rel_yaw is None:
    ...
    def scan_node_views(self, folder, initial_sweep):
        if initial_sweep:
            self.scan_360(folder)
            return
        self.scan_local_three_views(folder)
    ...
    def scan_local_three_views(self, folder):
        previous_tokens = None
        metadata = self.read_json(self.metadata_path(folder))
        try:
            responses = self.move_through_stops([5], reason=f"{folder.name}: local scan camera front")
        except Exception as exc:
            responses = []
            self.get_logger().warn(f"{folder.name}: could not pre-position camera front: {exc}")
        metadata["local_scan_setup"] = {
            "created_at": datetime.now().isoformat(),
            "mode": "local_three_view_scan",
            "views": LOCAL_SCAN_VIEWS,
            "camera_front_stop_index": 5,
            "camera_front_stop_name": "front",
            "camera_front_responses": responses,
            "settle_before_sweep_sec": self.settle_before_sweep_sec,
        }
        self.write_json(self.metadata_path(folder), metadata)
        ...
        if self.settle_before_sweep_sec > 0.0:
            self.get_logger().info(
                f"{folder.name}: waiting {self.settle_before_sweep_sec:.2f}s "
                "before local three-view capture"
            )
            self.sleep_seconds(self.settle_before_sweep_sec)
        ...
        for i, view in enumerate(LOCAL_SCAN_VIEWS, start=1):
            stop_index = int(view["stop_index"])
            stop_name = str(view["stop_name"])
            relative_yaw_deg = float(view["relative_yaw_deg"])
            ...
            self.get_logger().info(
                f"{folder.name}: local view {i} - {stop_name} "
                f"({relative_yaw_deg:+.0f} deg)"
            )
            move_started_at = self.get_clock().now()
            move_response = self.move_lerobot_stop(stop_index, stop_name)
            self.sleep_seconds(self.settle_after_motion_sec)
            ...
            if not self.wait_until_realsense_available(timeout_sec=self.wait_for_fresh_frame_sec):
                raise RuntimeError(f"Timed out waiting for RealSense frames at local view {stop_name}")
            ...
            if self.wait_for_fresh_color_frame_sec > 0.0:
                ...
                if not fresh:
                    self.get_logger().warn(
                        f"Timed out waiting for a fresh post-move color frame at "
                        f"local view {stop_name}; saving latest cached frame."
                    )
            ...
            self.append_snapshot_metadata(
                folder=folder,
                label=f"{view['label']}_{self.safe_name(stop_name)}",
                stop_index=i,
                stop_name=stop_name,
                move_response=move_response,
                filenames=filenames,
                ...
                camera_stop_index=stop_index,
                camera_stop_name=stop_name,
            )

        metadata = self.read_json(self.metadata_path(folder))
        try:
            responses = self.move_through_stops([5], reason=f"{folder.name}: keeping camera front")
            metadata["return_home"] = {
                "returned_at": datetime.now().isoformat(),
                "ok": True,
                "default_stop_index": 5,
    ...
    def write_initial_metadata(self, folder, node_id, scan_mode="initial_360_sweep"):
        ...
        metadata = {
            ...
            "scan_mode": scan_mode,
            "local_scan_views": LOCAL_SCAN_VIEWS if scan_mode == "local_three_view_scan" else None,
    ...
    def append_snapshot_metadata(
        ...
        camera_stop_index=None,
        camera_stop_name=None,
    ):
        path = self.metadata_path(folder)
        metadata = self.read_json(path)
        snapshot = {
        ...
        if relative_yaw_deg is not None:
            snapshot["relative_yaw_deg"] = float(relative_yaw_deg)
        if camera_stop_index is not None:
            snapshot["camera_stop_index"] = int(camera_stop_index)
        if camera_stop_name is not None:
            snapshot["camera_stop_name"] = str(camera_stop_name)
    ...
    def rotate_in_place_to_yaw(self, target_yaw):
        target_yaw = normalize_rad(float(target_yaw))
        start = self.get_clock().now()
        speed_limit = max(0.05, abs(float(self.rotation_cmd_angular_speed_rad_s)))
        min_speed = min(0.12, speed_limit)

        self.get_logger().info(
            f"Rotating in place before next-node goal: target yaw "
        ...
        while rclpy.ok():
            ...
            if pose is None:
                self.stop_robot()
                self.get_logger().warn("Pre-goal rotation skipped: robot pose unavailable.")
                return False

            error = normalize_rad(target_yaw - float(pose["yaw"]))
            if abs(error) <= self.rotation_yaw_tolerance_rad:
                self.stop_robot()
                self.get_logger().info(
                    f"Pre-goal rotation complete with yaw error "
            ...
            if elapsed > self.rotation_timeout_sec:
                self.stop_robot()
                self.get_logger().warn(
                    f"Pre-goal rotation timed out with yaw error "
                    f"{math.degrees(error):.1f} deg"
                )
                return False
            ...
            cmd.angular.z = math.copysign(
                min(speed_limit, max(min_speed, abs(error) * 0.8)),
                error,
            )
    ...
    def write_state(self, initialization_complete):
        payload = {
            ...
            "initial_sweep_completed": self.initial_sweep_completed,
            "next_scan_mode": (
                "local_three_view_scan"
                if self.initial_sweep_completed
                else "initial_360_sweep"
            ),
