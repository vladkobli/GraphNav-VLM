...
class ClipSegDepthNode(Node):
    def __init__(self):
        ...
        for prompt in self.prompts:
            ...
            topic = f"clipseg/masked_depth/{safe_name}"
            self.masked_depth_pubs[prompt] = self.create_publisher(Image, topic, 1)
            self.get_logger().info(f"Publishing masked depth for '{prompt}' on: /{topic}")
        ...
        try:
            ...
            for i, prompt in enumerate(self.prompts):
                ...
                if depth_encoding == "16UC1":
                    prompt_masked_depth = np.zeros_like(cv_depth_raw, dtype=np.uint16)
                    prompt_masked_depth[mask_bin] = cv_depth_raw[mask_bin]
                elif depth_encoding in ("32FC1", "32FC1_s"):
                    prompt_masked_depth = np.zeros_like(cv_depth_raw, dtype=np.float32)
                    prompt_masked_depth[mask_bin] = cv_depth_raw[mask_bin]
                else:
                    self.get_logger().warn(f"Unsupported depth encoding for prompt mask: {depth_encoding}")
                    prompt_masked_depth = np.zeros_like(cv_depth_raw, dtype=cv_depth_raw.dtype)
                ...
                if depths.size > 0:
                    ...
                    self.get_logger().info(f"[{prompt}] median distance: {dist_median:.2f} m")
            ...
            self.get_logger().error(f"Exception in synced_callback: {e}")
            import traceback
